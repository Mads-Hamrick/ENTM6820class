...
``` r
head(filter(microbiome.fungi2, Treatment == "Conv." & Fungicide == "C")) #variable value & variable value
```
...
``` r
head(filter(microbiome.fungi2, Sample == "A" | Sample == "B")) #variable value OR variable value
```
...
``` r
microbiome.fungi2$logRich <- log(microbiome.fungi2$richness)    #Creating a new column called logRich using $ symbol
...
``` r
head(mutate(microbiome.fungi2, Crop_Treatment = paste(Crop, Treatment)))    #Creating a new column which combines Crop and Treatment 
```
...
microbiome.fungi %>%    #DATA
  select(SampleID, Crop, Compartment:Fungicide, richness) %>%     #selecting out columns
  filter(Treatment == "Conv.") %>%    #filtering out the data for a specific requirement
  mutate(logRich = log(richness)) %>%     #creating a new column for the log(richness)
  head() 
```
...
![](2025_3_20_DataWrangling_Homework_mer0127_files/figure-html/unnamed-chunk-14-1.png)<!-- -->
